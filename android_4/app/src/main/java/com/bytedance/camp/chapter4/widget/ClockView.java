...
//        int second = 0;
        ...
        float hourAngle = (hour + (float) minute / 60) * FULL_CIRCLE_DEGREE / 12;
        float minuteAngle = (minute + (float) second / 60) * FULL_CIRCLE_DEGREE / 60;
        int secondAngle = second * FULL_CIRCLE_DEGREE / 60;
        Log.i("1",Float.toString(hourAngle));

        hourAngle += 270;
        if (hourAngle >= FULL_CIRCLE_DEGREE) hourAngle -= FULL_CIRCLE_DEGREE;
        Log.i("2",Float.toString(hourAngle));
        minuteAngle += 270;
        if (minuteAngle >= FULL_CIRCLE_DEGREE) minuteAngle -= FULL_CIRCLE_DEGREE;
        secondAngle += 270;
        if (secondAngle >= FULL_CIRCLE_DEGREE) secondAngle -= FULL_CIRCLE_DEGREE;
        ...
        length = radius * HOUR_NEEDLE_LENGTH_RATIO;
        radians = Math.toRadians(hourAngle);
        stopX =  (float) (centerX + length * Math.cos(radians));
        stopY =  (float) (centerY + length * Math.sin(radians));
        ...
        length = radius * MINUTE_NEEDLE_LENGTH_RATIO;
        radians = Math.toRadians(minuteAngle);
        stopX =  (float) (centerX + length * Math.cos(radians));
        stopY =  (float) (centerY + length * Math.sin(radians));
        ...
        length = radius * SECOND_NEEDLE_LENGTH_RATIO;
        radians = Math.toRadians(secondAngle);
        stopX =  (float) (centerX + length * Math.cos(radians));
        stopY =  (float) (centerY + length * Math.sin(radians));
    ...
    private void drawTimeNumbers(Canvas canvas) {
        ...
        centerX = getWidth() / 2f;
        centerY = getHeight() / 2f;
        float l = radius * 0.87f;
        numberPaint.setTextSize(radius * 0.13f);

        for (int i = 1; i <= 12; i++) {
            String timeNum = i + "";
            int angle = i * FULL_CIRCLE_DEGREE / 12;
            angle += 270;
            if (angle >= FULL_CIRCLE_DEGREE) angle -= FULL_CIRCLE_DEGREE;
            double radians = Math.toRadians(angle);
            float X = (float) (centerX + l * Math.cos(radians));
            float Y = (float) (centerY + l * Math.sin(radians));
            Y += numberPaint.getTextSize() / 2.5;
            canvas.drawText(timeNum, X, Y, numberPaint);
        }
